...
```mermaid
flowchart TB
    Boot["BlogModule.php<br/>(DI Container Bootstrap)<br/>27 services registered"]

    Boot --> Admin["Admin UI<br/>admin/*.php<br/>14 pages"]
    Boot --> Frontend["Frontend<br/>index.php<br/>5 pages"]
    Boot --> API["REST API<br/>(via Pipeline)"]

    API --> MW["Middleware<br/>RateLimit → Auth → Permission"]
    MW --> Controller["PostApiController"]

    Admin --> Repos
    Frontend --> Repos
    Controller --> Repos

    Repos["Repositories<br/>PostRepository &nbsp; CategoryRepository<br/>(lifecycle hooks: audit, events, tags)"]

    Repos --> Entities["Entities<br/>Post, Category<br/>(plain PHP 8.4)"]
    Repos --> EventBus["EventBus<br/>7 domain events"]
    Repos --> Infra["XMF Infrastructure<br/>Versioning, Audit<br/>Queue, Cache"]
```
